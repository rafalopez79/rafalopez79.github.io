...
//	var centerh = ch/2;
//	var delta = 0.5;
//	spot.x = spot.x + centerw < centerw ? spot.x + delta : spot.x - delta;
//	spot.y = spot.y + centerh < centerh ? spot.y + delta : spot.y - delta;
...
//		ctx.beginPath();
//		ctx.arc(centerw - radius/2, centerh - radius/2, radius, 0, Math.PI * 2, false);	
//		ctx.moveTo(centerw,centerh);
//		ctx.arc(centerw - radius/2, centerh - radius/2, radius/2, 0, Math.PI * 2, false);
//		ctx.closePath();
//		ctx.stroke();
		...
		for (var i = 0, num = points.length; i < num; i++) {
			...
			if (x < 0 || x > canvas.width || y < 0 || y > canvas.height) {
				points.splice(i, 1);
				points.push(createPointScoped(canvas, 2*canvas.width / 3.0, 2*canvas.height / 3.0));
			}			
		}
